...
def show_cm_via_ax(ax, cm, target_names, n_classes = 2, title = 'Confusion Matrix'):

    cm_df = pd.DataFrame(cm, columns = target_names, index = target_names)

    _ = sns.heatmap(cm_df, annot=True, ax=ax)
    ax.set_title(title)
    pass


def show_cm_via_plot(cm, target_names, n_classes = 2, title = 'Confusion Matrix'):
    _ = plt.figure()

    cm_df = pd.DataFrame(cm, columns = target_names, index = target_names)

    _ = sns.heatmap(cm_df, annot=True)
    plt.title(title)
    plt.show()
    pass


def show_cm(y_test, y_pred, target_names, n_classes = 2, title = 'Confusion Matrix', ax = None):
    ...
    if ax is None:
        show_cm_via_plot(cm = cm, target_names = target_names, n_classes = n_classes, title = title)
    else:
        show_cm_via_ax(ax = ax, cm = cm, target_names = target_names, n_classes = n_classes, title = title)
        pass
    pass
...
def show_roc_curve_via_ax(ax, fpr, tpr,  auc, title = 'Roc Curve'):

    ax.set_xlim(0, 0.2)
    ax.set_ylim(0.8, 1)

    ax.plot([0, 1], [0, 1], 'k--')
    ax.plot(fpr, tpr, label=f'AUC({auc * 100:.2f}%)')
...
def show_roc_curve_via_plot(fpr, tpr,  auc, title = 'Roc Curve'):
    plt.figure()
...
def show_roc_curve(fpr, tpr,  auc, title = 'Roc Curve', ax = None):

    if ax is None:
        show_roc_curve_via_plot(fpr, tpr,  auc, title = title)
    else:
        show_roc_curve_via_ax(ax, fpr, tpr,  auc, title = title)
    pass
